...
/**
 * Verify Dealerware webhook signature (HMAC-SHA256 over payload.data).
 * @param {object} payload - Full webhook body
 * @param {string} secret - DEALERWARE_SECRET
...
async function handleDefleet(env, data, timestamp) {
  ...
  if (!rows || rows.length === 0) {
    ...
    return jsonResponse({ success: true, event: 'defleet', vin, warning: 'No active loaner found for this VIN' }, 200);
  }
